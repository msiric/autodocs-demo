...
**Changed:** Added `CircuitOpenError` (`CIRCUIT_OPEN`, 503) with `serviceName` and `retryAfterMs` metadata.
**Why:** Circuit breaker pattern added for external service calls to prevent cascading failures.
...
**Changed:** File index was missing 7 source files. Removed reference to non-existent `src/auth/middleware.ts`. Added `src/api/rate-limiter.ts`, `src/api/health.ts`, `src/errors/circuit-breaker.ts`, `src/auth/jwt-auth.ts`, `src/auth/permissions.ts`, `src/auth/api-keys.ts`, `src/auth/audit.ts`, `src/webhooks/dispatcher.ts`.
**Why:** Multiple PRs added new files. The original file index only listed 3 files and referenced a non-existent middleware file.
...
**Changed:** Added webhook events: `api_key.created`, `api_key.revoked`, `auth.failed` to the dispatcher's supported event list.
**Why:** API key lifecycle events and authentication failures need to be observable via webhooks.
